...
@st.cache_data
def load_data():
# cache-bust: 2025-07-16T12:33:38.892967
    import os
    BASE_DIR = os.path.dirname(__file__)
...
def show_chart(summary):
    ...
    fig.add_trace(go.Bar(
        ...
        name="Approved",
        marker_color="green",
        text=[f"{a} ({p:.1f}%)" if a > 0 else "" for a, p in zip(approved, approved_pct)],
        textposition="inside"
    ))
    ...
    fig.add_trace(go.Bar(
        ...
        name="Refused",
        marker_color="red",
        text=[f"{r} ({p:.1f}%)" if r > 0 else "" for r, p in zip(refused, refused_pct)],
        textposition="inside"
    ))

    # Annotate total above each stacked bar
    fig.update_layout(
        annotations=[
            dict(
                x=weeks[i],
                y=total[i],
                text=f"Total: {int(total[i])}",
                showarrow=False,
                yshift=10,
    ...
    fig.update_layout(
        barmode="stack",
        height=400,
        title="Visa Decisions per Week",
        xaxis=dict(
            tickangle=-45,
            tickfont=dict(size=10),
            range=[weeks.iloc[0], weeks.iloc[min(9, len(weeks)-1)]],
            fixedrange=False  # enable horizontal panning
        ),
        yaxis=dict(
            title="Number of Applications",
            fixedrange=True  # disable zooming on y-axis
        ),
        dragmode="pan",  # only allow panning
        showlegend=True,
